...
@interface SkillSailViewController ()<UITableViewDataSource,UITableViewDelegate,UIAlertViewDelegate>
{
    UIImageView *topNav;
    UIView *leftView;
    UILabel *huodongName;
...
@implementation SkillSailViewController
...
- (void)viewDidLoad
{
    ...
    UILabel *titleLabel = [MyControll createLabelWithFrame:CGRectMake(0, 0, 50, 35) title:@"技能出售" font:20];
...
-(void)commitShenqing
{
    ...
    [self StartLoading];
    NSString *urlstr = [NSString stringWithFormat:@"%@createmark?uid=%@&token=%@&tid=%@",SERVER_URL,uid,token,self.dataDic[@"id"]];
...
-(void)loadData
{
    ...
    NSString *token = [user objectForKey:@"token"];
    [self StartLoading];
    NSString *urlstr = [NSString stringWithFormat:@"%@jinenginfo?uid=%@&token=%@",SERVER_URL,uid,token];
