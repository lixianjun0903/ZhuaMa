...
@interface EduInfoViewController ()<UITableViewDataSource,UITableViewDelegate,MJRefreshBaseViewDelegate>
{
...
@implementation EduInfoViewController
...
- (void)viewDidLoad {
    ...
    [self loadData];
    [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(refreshData) name:@"EduInfoListRefresh" object:nil];
}
-(void)refreshData
{
    mpage =0;
...
-(void)addWorkInfo
{
    EduInfoDetailViewController *vc = [[EduInfoDetailViewController alloc]init];
...
{
     EduInfoTableViewCell*cell = [tableView dequeueReusableCellWithIdentifier:@"ID"];
    if (!cell) {
        cell = [[EduInfoTableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"ID"];
    }
...
-(void)loadData
{
    ...
    [self StartLoading];
    NSString *urlstr = [NSString stringWithFormat:@"%@edulist?uid=%@&token=%@&limit=10&page=%d",SERVER_URL,uid,token,mpage+1];
