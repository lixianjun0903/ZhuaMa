...
@interface MybaoliaoViewController ()<UITableViewDataSource,UITableViewDelegate,MJRefreshBaseViewDelegate,commentDelegate,ShareDelegate,PicShowDelegate,inputKeyboardDelegate>
{
    UIImageView *topNav;
    NSString *keyWord;
...
@implementation MybaoliaoViewController
...
- (void)viewDidLoad
{
    ...
    UILabel *titleLabel = [MyControll createLabelWithFrame:CGRectMake(0, 0, 50, 35) title:@"我的爆料" font:20];
...
-(void)loadData
{
    ...
    NSString *token = [user objectForKey:@"token"];
    [self StartLoading];
    NSString *urlstr = [NSString stringWithFormat:@"%@mybaoliao?uid=%@&token=%@&where=&limit=20&page=%d&flag=%d",SERVER_URL,uid,token,mpage+1,type];
